  ...
  class Create
    ...
    def call(customer_id:, shipping_information:, billing_information:, subscription_plan_id:)
      ActiveRecord::Base.transaction do
        ...
        subscription_id = yield create_customers_subscription(customer_id, subscription_plan_id)
        yield create_shipping_details(shipping_information, subscription_id)
        token = yield perform_payment(billing_information, price)
        yield mark_successful_subscription_payment(subscription_id)
        yield create_payment_details(token, subscription_id)
    ...
    def create_shipping_details(shipping_information, subscription_id)
      Success(shipping_details_repo.create(
                **shipping_information.to_h.merge(customers_subscription_id: subscription_id)
              ))
    end

    def load_subscription_plan_price(subscription_plan_id)
      return Failure(:subscription_plan_not_found) unless subscription_plans_repo
    ...
    def mark_successful_subscription_payment(subscription_id)
      customer_subscriptions_repo.set_last_successful_payment_date(
        customers_subscription_id: subscription_id,
        date: Date.current
      )
    ...
    def create_payment_details(token, subscription_id)
      payment_details_repo.create(customers_subscription_id: subscription_id, token: token)
